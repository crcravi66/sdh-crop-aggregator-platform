...
const AllCropsPage = () => {
  ...
  return (
    <div className="w-full min-h-screen flex flex-col items-center bg-white">
      {/* Status Bar
      <div className="flex items-center justify-between w-full h-[40px] bg-white px-4">
        <div className="text-sm font-medium">9:09</div>
        <div className="flex items-center gap-2">
          <img src="https://dashboard.codeparrot.ai/api/image/Z5HGrvA8XwfbJP6Y/system-2.png" alt="system status" className="h-4 w-auto" />
        </div>
      </div> */}

      {/* Navigation Bar */}
      <NavigationBar title={'All crops'} /> 
      {/* <div className="flex items-center w-full h-[53px] bg-white px-6">
        <button className="flex items-center justify-center w-4 h-4 mr-4">
          <img src="https://dashboard.codeparrot.ai/api/image/Z5HGrvA8XwfbJP6Y/icon-arr.png" alt="Back" className="w-4 h-4" />
        </button>
        <h3 className="text-[#343434] text-xl font-semibold font-['Poppins'] leading-7">
          All crops
        </h3>
      </div> */}
        ...
        {/* Tab Bar */}
        {/* <div className="w-full min-w-[375px] h-[34px] bg-white flex justify-between items-center px-6 border-t border-gray-200">
            {['Home', 'Search', 'Add', 'Messages', 'Profile'].map((label, index) => (
            <div key={index} className="flex flex-col items-center">
                <img 
                src={`https://dashboard.codeparrot.ai/api/image/Z5HGrvA8XwfbJP6Y/system${index === 0 ? '' : '-2'}.png`} 
                alt={label}
