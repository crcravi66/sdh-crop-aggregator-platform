      ...
      <div className=" mx-auto flex-col max-w-md px-2  flex justify-around ">
        ...
        <div className="my-6">
          ...
          <div className="relative my-4">
            <div className="w-full min-h-10 border border-[#E5E5E5] rounded-md px-3 py-2 flex flex-wrap gap-2">
              {selectedTags.map((tag, index) => (
                <div
                  key={index}
                  className="flex items-center bg-[#4A3AFF] text-white px-3 py-1 rounded-2xl text-xs"
                >
                  <span>{tag}</span>
                  <button
                    type="button"
        ...
        <div>
          ...
          <div className="grid grid-cols-4 gap-2 sm:gap-4">
            {Array.from({ length: 4 }).map((_, index) => (
              <div
                key={index}
                className="min-w-14 h-20 sm:h-24 flex items-center justify-center border border-[#E5E5E5] rounded-lg overflow-hidden"
              >
                {images[index]
                  ? (
                      <img
